...
function tree_component () {
	...
	return {
		...
		parseJSON : function (data) {
			...
			if(this.settings.rules.metadata && data[this.settings.rules.metadata]) {
				var metadataString = '{ ';
				var metadata = data[this.settings.rules.metadata];
				for(i in metadata) {
					metadataString += i + ": " + metadata[i] + ", "; 
				}
				metadataString = metadataString.substring(0, metadataString.length-2);
				metadataString += ' }';
				str += this.settings.rules.metadata + '="' + metadataString + '"';
			}
			str += ">";
					...
					$(document)
						...
						.bind("mouseup", function (event) {
							...
							if(_this.drag && _this.drag.parentNode && _this.drag.parentNode == $(_this.container).get(0)) {
								...
								if(_this.moveType) _this.moved(_this.container.find("li.dragged"), _this.moveRef, _this.moveType, false, (_this.check("copy", _this.container.find("li.dragged")) || _this.settings.rules.drag_copy == "on" || (_this.settings.rules.drag_copy == "ctrl" && event.ctrlKey) ) );
							...
							if(_this.isdown) {
								...
								if( cnt.get(0) != _this.container.get(0) && (!_this.settings.rules.multitree || !tree_component.inst[cnt.attr("id")].settings.rules.multitree) ) {
									if($(_this.drag).children("IMG").size() == 0) {
										$(_this.drag).append("<img style='position:absolute; " + (_this.settings.ui.rtl ? "right" : "left" ) + ":4px; top:0px; background:white; padding:2px;' src='" + _this.path + "../img/remove.png' />");
									}
									_this.moveType = false;
									_this.moveRef = false;
									$("#marker").hide();
								...
								if(event.target.tagName == "A" ) {
									...
									if(tree_component.inst[cnt.attr("id")].checkMove(_this.container.find("li.dragged"), $(event.target), mov)) {
										if(mov == "inside")	$("#marker").attr("src", _this.path + "../img/plus.gif").width(11);
										else {
											if(cnt.hasClass("rtl"))	{ $("#marker").attr("src", _this.path + "../img/marker_rtl.gif").width(40); }
											else					{ $("#marker").attr("src", _this.path + "../img/marker.gif").width(40); }
										}
										_this.moveType	= mov;
		...
		check : function (rule, nodes) {
			...
			if(rule != "dragrules" && this.settings.rules.use_inline && this.settings.rules.metadata) {
				...
				else if (this.get_node(nodes).parent().parent().is("li")) if(typeof this.get_node(nodes).parent().parent().metadata()[rule] != "undefined")  return this.get_node(nodes).parent().parent().metadata()[rule];
			}
		...
		open_branch : function (obj, disable_animation, callback) {
			...
			if(this.settings.data.async && obj.find("li").size() == 0) {
				var _this = this;
				obj.children("ul:eq(0)").remove().end().append("<ul><li class='last'><a style='background-image:url(" + _this.path + "../img/throbber.gif)' href='#'>" + (_this.settings.lang.loading || "Loading ...") + "</a></li></ul>");
		...
		create : function (type, obj, data, nodeId, icon ) {
			if(this.locked) return this.error("LOCKED");
			// NOTHING SELECTED
		...
		// REMOVE NODES
		remove : function(obj, forced) {
			if(this.locked) return this.error("LOCKED");
			if(obj) {
				obj = this.get_node(obj);
				if(obj.size()) {
					if(!this.check("deletable", obj) && forced != true) return this.error("DELETE: NODE NOT DELETABLE");
				...
				if(!this.check("deletable", this.selected) && forced != true) return this.error("DELETE: NODE NOT DELETABLE");
