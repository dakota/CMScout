 ...
 */
	function setup(&$Model, $config = array()) {
		...
		if ($Model->hasField($settings['scope'])) {
			//$data = $Model->getAssociated($settings['scope']);
			//$parent =& $Model->{$settings['scope']};
			$settings['scopeField'] = $settings['scope'];
			$settings['scope'] = $Model->alias . '.' . $settings['scope'];
		}
 ...
 */
	function afterSave(&$Model, $created) {
		...
		if (!$created && $__parentChange) {
			$this->settings[$Model->alias]['__parentChange'] = false;
			return $this->_setParent($Model, $Model->data[$Model->alias][$parent]);
		}
		
		if (!$created && $__scopeChange) {
			$this->settings[$Model->alias]['__scopeChange'] = false;
			// return $this->_setScope($Model, $Model->data[$Model->alias][$scopeField]);
		}
	}
/**
 ...
 */
	function beforeDelete(&$Model) {
		...
		if ($diff > 2) {
			...
			$deleteScope[]["{$Model->alias}.{$left} BETWEEN ? AND ?"] = array($data[$left] + 1, $data[$right] - 1);
			$deleteScope[][$scope] = $data[$scopeField];
			$Model->deleteAll($deleteScope);
		}
		$this->__sync($Model, $diff, $data[$scopeField], '-', '> ' . $data[$right]);
		return true;
	}
/**
 ...
 */
	function beforeSave(&$Model) {
		extract($this->settings[$Model->alias]);
		$scopeValue = $Model->data[$Model->alias][$scopeField];
		...
		if (!$Model->id) {
			if (array_key_exists($parent, $Model->data[$Model->alias]) && $Model->data[$Model->alias][$parent]) {
				$parentNode = $Model->find('first', array(
					'conditions' => array($Model->escapeField() => $Model->data[$Model->alias][$parent]),
				...
				$edge = $this->__getMax($Model, array($scope=>$scopeValue), $right, $recursive);
				$Model->data[$Model->alias][$left] = $edge + 1;
				$Model->data[$Model->alias][$right] = $edge + 2;
			}
		} elseif (array_key_exists($parent, $Model->data[$Model->alias]) || array_key_exists($scopeField, $Model->data[$Model->alias])) {
			if (isset($Model->data[$Model->alias][$parent]) && $Model->data[$Model->alias][$parent] != $Model->field($parent)) {
				$this->settings[$Model->alias]['__parentChange'] = true;
			}
			if (isset($Model->data[$Model->alias][$scopeField]) && $Model->data[$Model->alias][$scopeField] != $Model->field($scopeField)) {
				$this->settings[$Model->alias]['__scopeChange'] = true;
			}
			if (array_key_exists($parent, $Model->data[$Model->alias]))
			{
				if (!$Model->data[$Model->alias][$parent]) {
					...
					list($node) = array_values($Model->find('first', array(
						'conditions' => array($Model->escapeField() => $Model->id),
 ...
 */
	function childcount(&$Model, $id = null, $direct = false) {
		...
		if ($direct) {
			return $Model->find('count', array('conditions' => array($scope=>$scopeValue, $Model->escapeField($parent) => $id)));
		}

		if ($id === null) {
			return $Model->find('count', array('conditions' => array($scope=>$scopeValue)));
		} elseif (isset($Model->data[$Model->alias][$left]) && isset($Model->data[$Model->alias][$right])) {
			$data = $Model->data[$Model->alias];
		} else {
			$data = $Model->find('first', array('conditions' => array($scope=>$scopeValue, $Model->escapeField() => $id), 'recursive' => $recursive));
			if (!$data) {
				return 0;
			}
 ...
 */
	function children(&$Model, $id = null, $direct = false, $fields = null, $order = null, $limit = null, $page = 1, $recursive = null) {
		...
		if ($direct) {
			$conditions = array($scope=>$scopeValue, $Model->escapeField($parent) => $id);
			return $Model->find('all', compact('conditions', 'fields', 'order', 'limit', 'page', 'recursive'));
		}

		if (!$id) {
			$conditions = array($scope=>$scopeValue);
		} else {
			$result = array_values($Model->find('first', array(
				'conditions' => array($scope=>$scopeValue, $Model->escapeField() => $id),
 ...
 */
	function getpath(&$Model, $id = null, $fields = null, $recursive = null) {
		...
		$scopeValue = $result[$Model->alias][$scopeField];
		if ($result) {
		...
		$results = $Model->find('all', array(
			'conditions' => array($scope=>$scopeValue, $Model->escapeField($left) . ' <=' => $item[$left], $Model->escapeField($right) . ' >=' => $item[$right]),
 ...
 */
	function movedown(&$Model, $id = null, $number = 1) {
		...
		$nextNode = $Model->find('first', array(
			'conditions' => array($scope=>$scopeValue, $Model->escapeField($left) => ($node[$right] + 1)),
			'fields' => array($Model->primaryKey, $left, $right), 'recursive' => $recursive)
		);
		if ($nextNode) {
		...
		$edge = $this->__getMax($Model, array($scope=>$scopeValue), $right, $recursive);
		$this->__sync($Model, $edge - $node[$left] + 1, $scopeValue, '+', 'BETWEEN ' . $node[$left] . ' AND ' . $node[$right]);
		$this->__sync($Model, $nextNode[$left] - $node[$left], $scopeValue, '-', 'BETWEEN ' . $nextNode[$left] . ' AND ' . $nextNode[$right]);
		$this->__sync($Model, $edge - $node[$left] - ($nextNode[$right] - $nextNode[$left]), $scopeValue, '-', '> ' . $edge);
 ...
 */
	function moveup(&$Model, $id = null, $number = 1) {
		...
		$previousNode = $Model->find('first', array(
			'conditions' => array($scope=>$scopeValue, $Model->escapeField($right) => ($node[$left] - 1)),
		...
		$edge = $this->__getMax($Model, array($scope=>$scopeValue), $right, $recursive);
		$this->__sync($Model, $edge - $previousNode[$left] +1, $scopeValue, '+', 'BETWEEN ' . $previousNode[$left] . ' AND ' . $previousNode[$right]);
		$this->__sync($Model, $node[$left] - $previousNode[$left], $scopeValue, '-', 'BETWEEN ' .$node[$left] . ' AND ' . $node[$right]);
		$this->__sync($Model, $edge - $previousNode[$left] - ($node[$right] - $node[$left]), $scopeValue, '-', '> ' . $edge);
		if (is_int($number)) {
			$number--;
		}
		if ($number) {
 ...
 */
	function recover(&$Model, $mode = 'parent', $missingParentAction = null) {
		...
		if ($mode == 'parent') {
			...
			$distinctScopes = $Model->find('all', array('fields' => array('distinct('.$scope.') as `Scope`'), 'order' => $left));

			foreach($distinctScopes as $distinctScope)
			{
				$searchScope = array($scope => $distinctScope[$Model->alias]['Scope']);
			
				$count = 1;
				foreach ($Model->find('all', array('conditions' => $searchScope, 'fields' => array($Model->primaryKey), 'order' => $left)) as $array) {
					$Model->id = $array[$Model->alias][$Model->primaryKey];
					$lft = $count++;
					$rght = $count++;
					$Model->save(array($left => $lft, $right => $rght), array('callbacks' => false));
				}
				foreach ($Model->find('all', array('conditions' => $searchScope, 'fields' => array($Model->primaryKey, $parent), 'order' => $left)) as $array) {
					$Model->create();
					$Model->id = $array[$Model->alias][$Model->primaryKey];
 ...
 */
	function removefromtree(&$Model, $id = null, $delete = false) {
		...
		if ($node[$right] == $node[$left] + 1) {
			...
			list($parentNode) = array_values($Model->find('first', array(
				'conditions' => array($scope=>$scopeValue, $Model->escapeField() => $node[$parent]),
				'fields' => array($Model->primaryKey, $left, $right),
				'recursive' => $recursive
			)));
		} else {
		...
		$this->__sync($Model, 1, $scopeValue, '-', 'BETWEEN ' . ($node[$left] + 1) . ' AND ' . ($node[$right] - 1));
		$this->__sync($Model, 2, $scopeValue, '-', '> ' . ($node[$right]));
		$Model->id = $id;
 ...
 */
	function verify(&$Model) {
		...
		$distinctScopes = $Model->find('all', array('fields' => 'distinct('.$scope.') as `Scope`', 'order' => $left));

		$errors =  array();

		foreach($distinctScopes as $distinctScope)
		{
			$scopeValue = $distinctScope[$Model->alias]['Scope'];
			$min = $this->__getMin($Model, array($scope=>$scopeValue), $left, $recursive);
			$edge = $this->__getMax($Model, array($scope=>$scopeValue), $right, $recursive);

			for ($i = $min; $i <= $edge; $i++) {
				$count = $Model->find('count', array('conditions' => array(
					array($scope=>$scopeValue), 'OR' => array($Model->escapeField($left) => $i, $Model->escapeField($right) => $i)
				)));
				if ($count != 1) {
					if ($count == 0) {
			...
			$node = $Model->find('first', array('conditions' => array(array($scope=>$scopeValue), $Model->escapeField($right) . '< ' . $Model->escapeField($left)), 'recursive' => 0));
			if ($node) {
			...
			foreach ($Model->find('all', array('conditions' => array($scope=>$scopeValue), 'recursive' => 0)) as $instance) {
				...
				} elseif ($Model->find('count', array('conditions' => array($scope=>$scopeValue, $Model->escapeField($left) . ' <' => $instance[$Model->alias][$left], $Model->escapeField($right) . ' >' => $instance[$Model->alias][$right]), 'recursive' => 0))) {
 ...
 */
	function _setParent(&$Model, $parentId = null, $created = false) {
		...
		list($node) = array_values($Model->find('first', array(
			'conditions' => array($Model->escapeField() => $Model->id),
			'fields' => array($Model->primaryKey, $parent, $left, $right, $scope),
			'recursive' => $recursive
		)));
		$scopeValue = $node[$scopeField];
		$edge = $this->__getMax($Model, array($scope=>$scopeValue), $right, $recursive, $created);

		if (empty ($parentId)) {
			$this->__sync($Model, $edge - $node[$left] + 1, $scopeValue, '+', 'BETWEEN ' . $node[$left] . ' AND ' . $node[$right], $created);
			$this->__sync($Model, $node[$right] - $node[$left] + 1, $scopeValue, '-', '> ' . $node[$left], $created);
		} else {
			...
			if (empty ($node[$left]) && empty ($node[$right])) {
				$this->__sync($Model, 2, $scopeValue, '+', '>= ' . $parentNode[$right], $created);
				$result = $Model->save(
				...
				if ($node[$left] > $parentNode[$left]) {
					if ($node[$right] < $parentNode[$right]) {
						$this->__sync($Model, $diff, $scopeValue, '-', 'BETWEEN ' . $node[$right] . ' AND ' . ($parentNode[$right] - 1), $created);
						$this->__sync($Model, $edge - $parentNode[$right] + $diff + 1, $scopeValue, '-', '> ' . $edge, $created);
					} else {
						$this->__sync($Model, $diff, $scopeValue, '+', 'BETWEEN ' . $parentNode[$right] . ' AND ' . $node[$right], $created);
						$this->__sync($Model, $edge - $parentNode[$right] + 1, $scopeValue, '-', '> ' . $edge, $created);
					}
				} else {
					$this->__sync($Model, $diff, $scopeValue, '-', 'BETWEEN ' . $node[$right] . ' AND ' . ($parentNode[$right] - 1), $created);
					$this->__sync($Model, $edge - $parentNode[$right] + $diff + 1, $scopeValue, '-', '> ' . $edge, $created);
 ...
 * @access private
 */
	function __sync(&$Model, $shift, $scopeValue = null, $dir = '+', $conditions = array(), $created = false, $field = 'both') {
		$ModelRecursive = $Model->recursive;
		extract($this->settings[$Model->alias]);
		$Model->recursive = $recursive;

		if ($field == 'both') {
			$this->__sync($Model, $shift, $scopeValue, $dir, $conditions, $created, $left);
			$field = $right;
		}
		if (is_string($conditions)) {
			$conditions = array("{$Model->alias}.{$field} {$conditions}");
		}
		if (($scope != '1 = 1' && $scope !== true) && $scope && $scopeValue !== null) {
			$conditions[] = array($scope=>$scopeValue);
		}
		if ($created) {
			$conditions['NOT'][$Model->alias . '.' . $Model->primaryKey] = $Model->id;
		}
		pr("SYNC CALLED. Shift: $shift, Scopevalue: $scopeValue, dir: $dir, created: $created, field: $field");
		pr(array($Model->alias . '.' . $field => $Model->escapeField($field) . ' ' . $dir . ' ' . $shift));
		pr($conditions);
