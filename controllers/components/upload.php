	...
	class UploadComponent extends Object {
		...
		function upload ($file, $destination, $name = NULL, $rules = NULL, $allowed = NULL) {
			...
			if (isset($file) && is_array($file) && !$this->upload_error($file['error'])) {
				...
				$fileName = ($name == NULL) ? $this->uniquename($this->_destination . $file['name']) : $this->_destination . $name;
				...
				if(!in_array($this->ext($this->_name),$this->_allowed)){
					...
					if (is_uploaded_file($fileTmp)) {
						...
						if ($rules == NULL) {
							// -- just upload it
							if (move_uploaded_file($fileTmp, $this->_name)) {
								chmod($output, 0644);
